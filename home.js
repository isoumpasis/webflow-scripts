...
function populateDescriptionSelect() {
	...
	if (selectedModelObj.isDirect) {
		...
		if (
			...
			(haveSameConsumption(filteredVehicles, { tolerance: 0.5 }) && haveSameEmulators(filteredVehicles))
			// &&
			// veh.cylinders < 6 &&
			// veh.hp < 190
		) {
...
=======
	...
	if (foundVehicleObj.hasOwnProperty('emulators') && foundVehicleObj.emulators[0] === 'B8') {
		...
		cobdContainer = document.querySelector('#suggested-cobd-8cyl');
		cobdContainer.querySelector('.suggested-cylinder-text').textContent = '4x2 = 8cyl';
		cobdContainer.querySelector('.suggested-cylinder-text-white').textContent = '4x2 = 8cyl έως 190HP';
		cobdContainer.style.display = 'grid';
	} else if (cyls == 5 || cyls == 6) {
		suggestedSystems = ['C-OBD II 6cyl'];
		const cobdDiv = document.querySelector('#suggested-cobd-6cyl');
...
>>>>>>> 73261fbff125aaa19206cd6eec0f43f37b2db71a
}
