...
const minNoVehicleSliderText = document.querySelector('.min-no-vehicle-slider-text');
const maxNoVehicleSliderText = document.querySelector('.max-no-vehicle-slider-text');
...
function noVehicleChangeMinMaxLabelsWeight() {
  maxNoVehicleSliderText.style.fontWeight =
    noVehicleNoCreditSlider.value === noVehicleNoCreditSlider.max ? 'bold' : 'normal';
  minNoVehicleSliderText.style.fontWeight =
...
maxDoseisSliderText.addEventListener(
  'click',
  e => doseisNoCreditSliderOnChange(doseisNoCreditSlider.max) //τεστ
);
...
function showDirectResults(fetchedModelObj) {
  ...
  if (foundVehicleObj.isConvertible) {
    ...
    directSystemDiv.querySelector('.di-engine-code-overlay').textContent =
      temp[1] + ' - ' + temp[0].replace(' ', ''); //descriptionSelect.value.split(' - ')[1];
...
function showCylinderResults(fetchedModelObj, years) {
  ...
  if (foundVehicleObj.hasOwnProperty('emulators') && foundVehicleObj.emulators[0] === 'B8') {
    ...
    cobdExrDiv
      ...
      .forEach(el => (el.textContent = '2-4cyl' + cylinderDescrText));
...
function configureSystemsEasyPay() {
  ...
  const suggestedPrices = [...activeContainer.querySelectorAll('.suggested-price')].map(
...
function configureNoCreditSliders() {
  ...
  document.querySelector('.enapomeinan-poso').textContent = (
    selectedEasyPaySystemPrice - parseInt(prokatavoliNoCreditSlider.value)
  ).toFixed(1);
...
function configureNoCreditResults() {
  ...
  const monthlyCost = -PMT(
    ...
    doseisSliderValueInt,
    parseFloat(enapomeinanPoso.textContent)
  );
  noCreditMonthlyCost.textContent = monthlyCost.toFixed(2) + '€';
